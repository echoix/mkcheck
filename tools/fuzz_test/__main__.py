    ...
    def __init__(self, root, graph, rule_file, use_hash, args):
        ...
        with open(os.devnull, "w") as devnull:
            code = subprocess.Popen(
                ["make", "--dry-run", "clean"], stdout=devnull, stderr=devnull, cwd=root
            ).wait()
...
def fuzz_test(project, files):
    ...
    count = len(fuzzed)
    for idx, input in zip(list(range(count)), fuzzed):
...
def main():
    ...
    parser.add_argument(
        "--graph-path", type=str, default="/tmp/mkcheck", help="Path to the graph file"
    )
    parser.add_argument(
        "cmd",
        metavar="COMMAND",
    ...
    parser.add_argument(
        "files", metavar="FILES", type=str, nargs="*", help="Input files"
    )
    parser.add_argument("--rule-path", type=str, help="Path to the rule file")
    parser.add_argument(
        "--use-hash",
        action="store_true",
    ...
    parser.add_argument(
        "--argv", type=str, default="", help="Additional arguments to make"
    )
